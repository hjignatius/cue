...
<table>
  ...
  <tr><td><strong>Time signature</strong></td><td>A picker offering <strong>4/4, 3/4, 2/4, 2/2, 6/8, 9/8, 12/8, 5/4</strong> and <strong>7/8</strong> — stored with the song and used by the metronome in Present mode. (It used to be a 4/4 ↔ 3/4 toggle.) A signature that arrived from an imported file but isn't on the list is shown as-is and left alone.</td></tr>
...
<p>The <strong>Chords</strong> panel shows a chord diagram for every chord detected in the current song, drawn for the instrument chosen in <strong>Settings → Chord instrument</strong> — Ukulele or Baritone (4 strings) or Guitar (6 strings). There is no separate switch for it: the panel is shown whenever an instrument is set, and setting the instrument to <strong>None</strong> hides it entirely. On a portrait phone it is one of the three panels on the <strong>Text / Preview / Chords</strong> selector instead.</p>
<ul>
...
<h3>Long lines</h3>
<p>When a line is wider than the screen, Cue wraps it and continues on the next line, <strong>indented</strong> so you can see at a glance that it's the rest of the line above rather than a new one. It breaks <strong>between words</strong>, keeping a chord welded to the syllable it sits above — so a chord written inside a word, like <code>won[G]derful</code>, can never split the word across two lines. The same applies to exported PDFs. A single stretch with no spaces in it that is wider than the screen is the one case that still can't be broken anywhere.</p>
...
<p><strong>QR code.</strong> The Share dialog has a <strong>Show QR code</strong> button under the link. It draws the same link as a code anyone in the room can scan with a phone camera — handy at a jam or a workshop, where reading a link aloud is hopeless. It is generated on your device (nothing is sent anywhere to make it), and it is always drawn black-on-white even in dark mode, because that is what phone cameras can actually read. If you <strong>Stop sharing</strong>, the code disappears along with the link. Scanning it opens the same shared-set page the link does.</p>
<p><strong>Save as PDF</strong> under the code writes a one-page A4 sheet — the set name, the code at about 11cm square, and the link spelled out underneath for anyone whose camera won't cooperate. Print it and prop it on a stand or pin it up. The code is drawn as vector art rather than a picture of one, so it stays sharp at any print size.</p>
<p><strong>A shared set now opens without a connection.</strong> Each time you open one it is saved on your device, so if you arrive somewhere with no signal the songs are still there. A banner says <em>"No connection — showing the copy you last opened today"</em> (or the date), with a <strong>Retry</strong> button for when the signal returns — the publisher may have changed the set since. PDF lead sheets come through too, as long as they finished downloading the first time you opened it. Two things deliberately do <em>not</em> work from the saved copy: a set that has been <strong>unshared</strong> shows as unavailable rather than reappearing from your device, and pulling updates needs the network by definition.</p>
...
{duration: 3:06}
{timesig: 3/4}</pre>
<p>The <code>{timesig:}</code> directive is written whenever the song is in anything other than 4/4 — 4/4 is the default and is omitted to keep files compatible with other apps. An incoming <code>{timesig:}</code> is accepted in any form and kept.</p>
