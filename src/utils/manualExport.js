...
<table>
  ...
  <tr><td><strong>Fill in song details</strong></td><td>Reads the chart and suggests Title, Artist, Key (from the chords), plus Tempo, Duration and a real YouTube link (web-searched). Nothing changes until you apply it. Every suggestion has a <strong>tick box</strong>, all ticked to begin with — untick any you don't want (a YouTube link you'd sooner choose yourself, say) and the button applies only the rest, naming the count. Each row also has its own <strong>Apply</strong>, which is the quickest way to take just one.</td></tr>
