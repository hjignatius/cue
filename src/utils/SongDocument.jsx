...
// Embed Unicode-capable fonts so arrows, symbols and smart quotes render (the
// built-in Courier/Helvetica can't). Idempotent, safe to call at module load.
registerPdfFonts();
...
function buildStyles(scale, chordColor = '#4f46e5') {
  ...
  return StyleSheet.create({
    page:           { paddingTop: 48, paddingRight: PAGE_PAD_X, paddingBottom: 48, paddingLeft: PAGE_PAD_X, fontFamily: FONT_SANS, backgroundColor: '#ffffff' },
    header:         { marginBottom: 24 * s, borderBottomWidth: 1, borderBottomColor: '#cccccc', paddingBottom: 12 * s },
    title:          { fontSize: 22 * s, fontFamily: FONT_SANS, fontWeight: 'bold', color: '#1a1a2e', marginBottom: 4 * s, textAlign: 'center' },
    ...
    markerText:     { color: chordColor, fontFamily: FONT_MONO, fontWeight: 'bold' },
...
function StyledRunsPdf({ runs, styles }) {
  return (runs || []).map((r, i) => {
    ...
    const style = { fontFamily: FONT_MONO };
